...
const BASE64_FILEPATH = 'features/shared-objects/base64value/';
...
Then('Result should be {string}', async function (result) {
  ...
  let verifyData = await readFileAsync(BASE64_FILEPATH + result + '.txt');

  assert(data.length == verifyData.length);
  assert(data == verifyData);
})
